...
const App: React.FC = () => {
  ...
  }, [messages, isGenerating]); // Also scroll while generating
  ...
  const handleSendMessage = async () => {
    ...
    if (!process.env.API_KEY) {
      ...
      const errorMsg: Message = {
        ...
        role: 'model',
        text: "I seem to be missing my API key. Please configure it in your environment to proceed.",
        timestamp: Date.now()
      };
    ...
    try {
      ...
      for await (const chunk of stream) {
        if (stopGenerationRef.current) break; // User stopped generation

        const chunkText = chunk.text || '';
        fullText += chunkText;
        
        // Extract Grounding Metadata if present in any chunk
      ...
      console.error(error);
      if (!stopGenerationRef.current) { // Only show error if not manually stopped
          setOrbState(OrbState.ERROR);
          playSound('error');
          ...
          if (error.message) {
            if (error.message.includes('429')) {
              errorMessage = "I'm getting too many requests at once. Give me a moment to cool down.";
            } else if (error.message.includes('SAFETY') || error.message.includes('blocked')) {
              errorMessage = "I can't respond to that due to my safety protocols.";
            }
          }
          
          // Update the placeholder message to contain the error
          setMessages(prev => prev.map(msg => 
            msg.id === botMsgId 
  ...
  const handleSpeak = (text: string) => {
    ...
    setOrbState(OrbState.RESPONDING); // Trigger visualizer
    utterance.onend = () => setOrbState(OrbState.IDLE);
    utterance.onerror = () => setOrbState(OrbState.IDLE);
        ...
        <div className="w-full max-w-2xl relative flex items-center gap-2">
          ...
          <div className="flex-1 relative">
            <input
              ...
              disabled={isGenerating}
              className={`w-full p-3.5 pl-4 pr-12 rounded-full bg-white dark:bg-[#1A1D2D] border border-gray-200 dark:border-gray-700 focus:border-purple-500 dark:focus:border-purple-500 focus:ring-2 focus:ring-purple-500/20 outline-none shadow-lg shadow-black/5 dark:shadow-black/20 text-gray-800 dark:text-white placeholder-gray-400 transition-all ${orbState === OrbState.LISTENING ? 'ring-2 ring-red-500/50 border-red-500' : ''} ${isGenerating ? 'opacity-70 cursor-not-allowed' : ''}`}
            />
            ...
            {isGenerating ? (
               <button 
                 ...
                 className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-full bg-red-500 text-white transition-all hover:bg-red-600 hover:scale-110 hover:shadow-[0_0_10px_rgba(239,68,68,0.6)] active:scale-95"
